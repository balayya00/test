...
def fmt_year(start, end=None, status=None):
    ...
    if running: return f'{s}–{CURRENT_YEAR}'
    if not e or e == s: return s
    return f'{s}–{e}'
...
def tmdb_tv(show_id, show_name=None):
    ...
    created_by = [p['name'] for p in details.get('created_by', []) if p.get('name')]
    ...
    dbg(f'  year={year} synopsis={"YES" if synopsis else "NO"} '
        f'cast={cast} genres={genres}')
...
def _get_chromium_args():
    """
    Extra args needed for Render/Docker environments.
    Detects if running in container and adds necessary flags.
    """
    args = [
        '--no-sandbox',
    ...
    # Extra flags for Render's environment
...
async def scrape_serializd():
    ...
    for pattern in possible_paths:
        ...
        if matches:
            ...
            dbg(f'  Found chromium at: {chromium_path}')
            break
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            print('  Trying with default chromium path...')
            # Try without explicit path
            launch_opts.pop('executable_path', None)
            try:
                browser = await p.chromium.launch(**launch_opts)
            except Exception as e2:
                print(f'  Playwright launch failed again: {e2}')
                return []
        ...
        try:
            ...
            await page.wait_for_timeout(8000)  # longer wait on Render
            for i in range(15):               # more scrolls on Render
                await page.evaluate('window.scrollTo(0, document.body.scrollHeight)')
                await page.wait_for_timeout(2000)
                dbg(f'  scroll {i+1}/15  captured={len(captured)}')
                if len(captured) >= 3:        # stop early if we have enough
                    break
        except Exception as e:
...
def fetch_serializd():
    ...
    try:
        from playwright.async_api import async_playwright as _check
        dbg('  Playwright available')
    except ImportError:
        print('  Playwright not installed — serving from cache')
        cache   = load_pkl()
    ...
    try:
        ...
        if not raw:
            print('  Nothing captured — check if Playwright works on this environment')
            print('  Serving from existing cache')
            cache   = load_pkl()
            entries = list(cache.values()) if cache else load_json()
            save_json(entries)
            return entries
